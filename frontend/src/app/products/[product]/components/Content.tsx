...
import ContentSkeletonLoading from "./ContentSkeletonLoading";
...
const Content = () => {
  ...
  const {data: singleProduct, error: errorProduct, loading: isLoading} = useQuery(ProductsOperation.Queries.product,{
    variables: {
      productId
    }
  });
  ...
  // // const { data: product, error, isLoading } = useSWR(`https://api.npoint.io/412448615c4faa493df3/` + (+productId! - 1).toString(), fetcher)
  if (isLoading) 
